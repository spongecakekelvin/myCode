...
function DragItem:setEnableCallback(callback)
...
function DragItem:enable(state)
    self:setTouchEnable(state)
    self.m_enable = state
    if self.enableCallback then
        self.enableCallback(self, state)
    end
end

function DragItem:isEnable(state)
