...
-- lexer: text mode
function dotext (f, buf, addToBuff)
    local c = f:read(1)
    -- yzjprint("text c : ", c)
    if c == "<" then
        if buf:content() ~= "" then addToBuff(Text(buf:content())) end
        buf:set(c)
        dotag(f, buf, addToBuff)
    elseif c then
        buf:append(c)
        dotext(f, buf, addToBuff)
    else
        if buf:content() ~= "" then addToBuff(Text(buf:content())) end
    end
end
...
-- lexer: tag mode
function dotag (f, buf, addToBuff)
    local c = f:read(1)
    -- yzjprint("tag c : ", c)
    if c == ">" then
        do addToBuff(Tag(buf:append(c):content())) end
        buf:clear()
        dotext(f, buf, addToBuff)
    elseif c then
        buf:append(c)
        ...
        if buf:content() ~= "" then addToBuff(Tag(buf:content())) end
    end
end

-- replace corroutine by using closure
function makeBuffFunc()
    local ret = {}
    return function(i)
        if not i then
            return ret
...
function getContentTable(f)
    local pos = 1
    local buf = newbuf()
    local addToBuff = makeBuffFunc()

    while (not f:isEnded()) do
        dotext(f, buf, addToBuff)
    end
...
function parseHtml(f)
    ...
    local root = {_tag = "font", _attr = {}}
    local stack = {root}
    local contentTable = getContentTable(f)
    -- printTable(ret, yzjprint)

    for _, i in ipairs(contentTable) do
    -- for i in makeiter(function () return text(f, newbuf()) end) do -- corroutine is slow
...
local function copyTable(dest, src, labelname)
    -- tag <a> will not be replaced by <font>
    if labelname then
        local oldLabelName = dest.labelname
        if not (oldLabelName and noReplaceMap[oldLabelName]) then
            dest.labelname = labelname
        end
    end
...
-- flattern the table, outer attr will cover the inner
local simplifyTable
simplifyTable = function(tab)
    local ret = {}
    local attr = tab._attr
    local labelname = tab._tag

    if tab[1] then
      for i, v in ipairs(tab) do
          if type(v) == "string" then

              ret[#ret+ 1] = copyTable({content = v}, attr, labelname)

          elseif type(v) == "table" then

              local temp = simplifyTable(v)
              for j, k in ipairs(temp) do
                  ret[#ret+ 1] = copyTable(k, attr, labelname)
              end
          end
      end
    else
      ret[#ret+ 1] = copyTable({content = ""}, attr, labelname)
    end
...
function parse(s)
    ...
    for i = 1, string.len(s) do
        content[i] = string.sub(s, i, i)
    end
    local handle = {
        _content = content,
        _pos = 1,
        read = function (self, length)
            local ret = self._content[self._pos]
            self._pos = self._pos + length
            return ret
        end,
        isEnded = function(self)
            return (self._pos > #self._content)
        end
    }
    local ret = parseHtml(handle)
    -- flattern the table, outer attr will cover the inner
    ret = simplifyTable(ret)
    return ret
end
