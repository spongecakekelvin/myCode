...
static int loaderProgressFunc(void *ptr, double totalToDownload, double nowDownloaded, double totalToUpLoad, double nowUpLoaded)
{
	...
	{
		tmp = (curTask->startPos + nowDownloaded) / (curTask->startPos + totalToDownload) * 100;
	}else {
		tmp = nowDownloaded / totalToDownload * 100;
	}
...
/// callbacks
void PlFileLoader::onProgress(double percent)
{
	...
	if(_onLuaProgress){
		...
		if(defaultEngine){
			...
			lua_pushnumber(stack->getLuaState(), percent);
